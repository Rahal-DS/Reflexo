//
//  ReactionTimeLeaderboardView.swift
//  Reflexo
//
...
/// A SwiftUI screen that shows the **Reaction Time** global leaderboard.
///
/// ### Overview
/// ``ReactionTimeLeaderboardView`` fetches and displays the top scores for the
/// *Reaction Time* mini‑game. It uses ``LeaderboardViewModel`` to load results
/// from Firestore via ``FirebaseManager`` and presents a clean UI with loading,
/// error, empty, and content states.
///
/// The view performs a one‑shot fetch when it appears. Users can pull to refresh
/// to re‑query the latest leaderboard without leaving the screen.
///
/// ### Data Flow
/// 1. ``LeaderboardViewModel`` is created with `gameName: "ReactionTime"`.
/// 2. ``LeaderboardViewModel/load(fb:)`` queries Firestore once (no live listener).
/// 3. ``LeaderboardViewModel/topScores`` drives the list; each row is rendered
/// with ``DisplayScore`` and shows the user's display name and milliseconds.
///
/// ### States
/// - **Loading** → `ProgressView("Loading scores…")`
/// - **Error** → Visible message with a **Retry** button
/// - **Empty** → A friendly prompt when no scores exist
/// - **Content** → Scrollable ranked list of entries
///
struct ReactionTimeLeaderboardView: View {
    ...
    var body: some View {
        ZStack {
            ...
            VStack(spacing: 20) {
                Logo()
                HeaderView(title: "Reaction Time Leaderboard")
                contentView
            }
